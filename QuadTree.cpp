...
bool QuadTree::insert(point p) {
    ...
    for (vector<point>::iterator it = points.begin(); it != points.end(); ++it) {
        if (NE->insert(*it)) continue;
        if (NW->insert(*it)) continue;
        if (SE->insert(*it)) continue;
        if (SW->insert(*it)) continue;
    }
    points.clear();
...
vector<point> QuadTree::queryRange(rect r) {
    ...
    if (!boundary.intersects(r)) return pointsInRange; // No intersection
    for (vector<point>::iterator it = points.begin(); it != points.end(); ++it) {
        if (r.containsPoint(*it)) {
            pointsInRange.push_back(*it);
        }
    }
    if (NE == NULL) return pointsInRange; // No children
